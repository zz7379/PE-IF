...
if __name__ == '__main__':
    ...
    if clientID != -1:
        ...
        for i in range(0, 6):
            return_code, arm_joints_handle[i] = vrep_sim.simxGetObjectHandle(clientID, 'elfin_joint' + str(i+1),
                                                                             vrep_sim.simx_opmode_blocking)
            if (return_code == vrep_sim.simx_return_ok):
                print('get object arm joint ' + str(i+1) + ' ok.')
                _, arm_joint_pos = vrep_sim.simxGetObjectPosition(clientID, arm_joints_handle[i], -1,
                                                                  vrep_sim.simx_opmode_blocking)
