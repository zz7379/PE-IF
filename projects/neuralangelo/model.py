...
class Model(BaseModel):
    ...
    def __init__(self, cfg_model, cfg_data):
        ...
        self.to_full_val_image = partial(misc.to_full_image, image_size=cfg_data.test.image_size)
        self.cfg_data_wxz = cfg_data
    ...
    @torch.no_grad()
    def inference(self, data, ray_indices=None):
        # import ipdb; ipdb.set_trace()
        self.eval()

        # Render the full images.
        output = self.render_image(data["pose"], data["intr"], image_size=self.cfg_data_wxz.test.image_size, # TODO: may cause error validation image
                                   stratified=False, sample_idx=data["idx"], ray_indices=ray_indices)  # [B,N,C]
    ...
    def render_image(self, pose, intr, image_size, stratified=False, sample_idx=None, ray_indices=None, detach=True):
        ...
        """
        # if inerf_data is not None:

        output = defaultdict(list)
        ray_indices = ray_indices if ray_indices is not None else None
        for center, ray, _ in self.ray_generator(pose, intr, image_size, full_image=True, ray_indices=ray_indices):
            ...
            for key, value in output_batch.items():
                if value is not None:
                    output[key].append(value.detach() if detach else value)
    ...
    def render_rays_object(self, center, ray_unit, near, far, outside, app, stratified=False):
        ...
        # import ipdb; ipdb.set_trace() # why rgbs is 127 when extracting mesh??????
        rgbs = self.neural_rgb.forward(points, normals, rays_unit, feats, app=app)  # [B,R,N,3]
        if self.debugckpt:
            import ipdb;ipdb.set_trace()
            print('rgbs_mean', rgbs.mean())
