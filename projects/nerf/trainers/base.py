...
class BaseTrainer(BaseTrainer):
    ...
    def train(self, cfg, data_loader, single_gpu=False, profile=False, show_pbar=False):
        ...
        if ((self.current_epoch % self.cfg.validation_epoch == 0 or
            ...
            key='rgb_map';image = data_all[key][0];image = (image * 255).byte().permute(1, 2, 0).cpu().numpy();cv2.imwrite(f"outdir/evalloader_{key}.png", image)
            # Log the results to W&B.
            if is_master():
                self.log_wandb_scalars(data_all, mode="val")
                self.log_wandb_images(data_all, mode="val", max_samples=self.cfg.data.val.max_viz_samples)
        ...
        # data_all = self.test(self.test_data_loader, mode="test", show_pbar=show_pbar)
        # import ipdb; ipdb.set_trace()
        # data_all = self.test(self.eval_data_loader, mode="test", show_pbar=show_pbar)
    ...
    def test(self, data_loader, output_dir=None, inference_args=None, mode="test", show_pbar=False):
        ...
        if show_pbar:
            data_loader = tqdm(data_loader, desc="Evaluating" if mode == 'val' else "Testing", leave=False)
        data_batches = []
        for it, data in enumerate(data_loader):
            # import ipdb; ipdb.set_trace()
            data = self.start_of_iteration(data, current_iteration=self.current_iteration) # wxz comment this line
            data = to_cuda(data)
        ...
        data_gather = collate_test_data_batches(data_batches, mode=mode) # if mode == test, the drop "image" to save memory
        ...
        if is_master():
            ...
            if mode == "test":
                ...
                self.dump_test_results(data_all)
            return data_all
        else:
            return


    def dump_test_results(self, data_all, reverse_xy=False):
        output_dir = "outdir"
        results = dict(
            image=preprocess_image(data_all["rgb_map"]),
            inv_depth=preprocess_image(data_all["depth_map"]),
            normal=preprocess_image(data_all["normal_map"]),
        )
        ...
        for key, image_list in results.items():
            ...
            video = cv2.VideoWriter(video_fname ,cv2.VideoWriter_fourcc(*'mp4v'),25,(image_list[0].shape[1], image_list[0].shape[0]))
            for i, image in enumerate(image_list):
                ...
                if reverse_xy:
                    image = image[::-1, ::-1, :]
                if "file_prefix" in data_all.keys():
                    cv2.imwrite(f"{output_dir}/{data_all['file_prefix'][i]}_{key}{idx}.png", image)
                else:
                    cv2.imwrite(f"{output_dir}/{key}{idx}.png", image)
    ...
    @torch.no_grad()
    def inference_one_image(self, data, reverse_xy=False, ray_indices=None):
        ...
        data = to_cuda(data)
        output = model.inference(data, ray_indices)
        data.update(output)
        data_batches.append(data)
        ...
        if is_master():
            ...
            self.dump_test_results(data_all, reverse_xy=reverse_xy)
            return data_all
        else:
            return