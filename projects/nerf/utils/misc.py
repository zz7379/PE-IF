...
def collate_test_data_batches(data_batches, mode="val"):
    ...
    with torch.no_grad():
        ...
        for key in data_batches[0].keys():
            if key == "file_prefix":
                ...
                data_gather[key] = [data["file_prefix"] for data in data_batches]
                continue
            if key == "image" and mode == "test":
                continue
...
def get_unique_test_data(data_gather, idx):
    ...
    for key, value in data_gather.items():
        if isinstance(value, dict):
            ...
            # import ipdb; ipdb.set_trace()
            data_all[key] = [value[i] for i in idx]
        else:
            raise TypeError
    return data_all
