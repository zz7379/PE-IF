...
class BaseTrainer(object):
    ...
    def set_data_loader(self, cfg, split, shuffle=True, drop_last=True, seed=0):
        ...
        if split == "train":
            ...
            # self.test_data_loader = get_test_dataloader(cfg, seed=seed)
...
class Checkpointer(object):
    ...
    def save(self, current_epoch, current_iteration, latest=False):
        ...
        if self.debugckpt or True:
            # for i in save_dict['model'].keys():
            #     if 'neural_rgb' in i:
            #         print(i)
            print("save: ", save_dict['model']['module.neural_rgb.mlp.linears.4.bias'])
            # import ipdb; ipdb.set_trace()
            # Save to local disk.
            ...
            print(checkpoint_path, " is ", state_dict['model']['module.neural_rgb.mlp.linears.4.bias'])
            if not state_dict['model']['module.neural_rgb.mlp.linears.4.bias'][0] == save_dict['model']['module.neural_rgb.mlp.linears.4.bias'][0]:
                torch.save(save_dict, checkpoint_path)
                state_dict = torch.load(checkpoint_path)
                # import ipdb; ipdb.set_trace()
                if not state_dict['model']['module.neural_rgb.mlp.linears.4.bias'][0] == save_dict['model']['module.neural_rgb.mlp.linears.4.bias'][0]:
        ...
        if checkpoint_path is not None:
            ...
            if self.debugckpt:
                for i in state_dict['model'].keys():
                    if 'neural_rgb' in i:
                        print(i)
                print(state_dict['model']['module.neural_rgb.mlp.linears.4.bias'])
                # import ipdb; ipdb.set_trace()
